...
int main(int argc, char **argv)
{
    ...
    if (rank == 0) {
        ...
        MPI_Recv(msg_recv, MSG_SIZE, MPI_CHARACTER, 1, 1, MPI_COMM_WORLD, &status);
    }

    if (rank == 1) {
        MPI_Send(msg1, MSG_SIZE, MPI_CHARACTER, 0, 1, MPI_COMM_WORLD);
        MPI_Recv(msg_recv, MSG_SIZE, MPI_CHARACTER, 0, 1, MPI_COMM_WORLD, &status);
    }

    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Get_processor_name(processor_name, &namelen);
    printf("Process %d on %s out of %d recv=%s\n", rank, processor_name, numprocs, msg_recv);
    MPI_Finalize();
    return 0;
}
